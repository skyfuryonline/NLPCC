...
old_student, _ = FastLanguageModel.from_pretrained(
    model_name="/root/shared-nvme/model/unsloth/Qwen2.5-1.5B-bnb-4bit",  # 1.5B参数的千问模型
...
student, _ = FastLanguageModel.from_pretrained(
    model_name="/root/shared-nvme/results/checkpoint-930",  # 1.5B参数的千问模型
    max_seq_length=max_seq_length,
    dtype=dtype,
    load_in_4bit=load_in_4bit,  # 4bit量化加载
)
...
teacher, tokenizer = FastLanguageModel.from_pretrained(
    model_name="/root/shared-nvme/model/unsloth/Qwen2.5-7B",  # 7B参数的千问模型
    max_seq_length=max_seq_length,
    dtype=dtype,
    load_in_4bit=load_in_4bit,  # 4bit量化加载
)
