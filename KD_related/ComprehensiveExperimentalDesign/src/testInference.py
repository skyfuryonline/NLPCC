...
from config import max_seq_length,load_in_4bit
...
{}"""
# 定义待推理的输入文本或数据
input_data =alpaca_prompt.format(
    '''Summarize the following news article into 2-3 concise sentences, ensuring to include key details''',
...
# 转换为 token
inputs = tokenizer(input_data, return_tensors="pt").to(device)  # 假设使用 GPU

# 生成输出
outputs = model.generate(**inputs, max_new_tokens=200, temperature=0.75)
