...
# 加载 Summary 验证集（假设从 dataset 模块导入）
from ConstructDataForSummary import val_summary_dataset
val_dataset = val_summary_dataset.map(formatting_prompts_func, batched=True)
...
def evaluate_Summary_books(teacher, original_student, distilled_student, dataset, tokenizer, batch_size=4):
...
# 执行评估
results = evaluate_Summary_books(teacher, original_student, distilled_student, val_dataset, tokenizer)
print("评估结果（针对 Summary 数据集）：")
